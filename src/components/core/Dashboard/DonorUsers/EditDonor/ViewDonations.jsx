...
import { viewDonorDetails, deleteDonorDetails } from '../../../../../services/operations/donorAPI'
...
const ViewDonations = () => {
    ...
    //To Get All Donation Details Of The Donor
    const getDetails = async () => {
        try {
            const response = await viewDonorDetails(token)
            setDetails(response)
        }
        catch (error) {
    ...
    //For Deleting A Particular Donation Detail
    const handleDetailsDelete = async (donorId) => {
        setLoading(true)
        await deleteDonorDetails({ donorId: donorId }, token)
        const result = await viewDonorDetails(token)
        if (result) {
            setDetails(result)
        }
                                        ...
                                        >
                                            <Td className="flex flex-1">
                                                <div className="flex flex-col justify-between">
                                                    ...
                                                    <p className="text-sm font-medium text-richblack-300">
                                                        Description: {d.donationDescription.split(" ").length >
                                                            10
                                                    ...
                                                    <p className="text-sm font-medium text-richblack-300">
                                                        Delivery Option: {d.deliveryOption}
                                                    </p>
                                                    <p className="text-sm font-medium text-richblack-300">
                                            ...
                                            <Td className="text-sm font-medium text-richblack-300 ">
                                                <button
                                                    disabled={loading}
                                                    onClick={() => {
                                                        navigate(`/dashboard/edit-donor-details/${d._id}`)
                                                    }}
                                                    title="Edit"
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        setConfirmationModal({
                                                            text1: "Do You Want To Delete This Donation Detail?",
                                                            text2:
