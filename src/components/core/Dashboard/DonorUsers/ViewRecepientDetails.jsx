...
const ViewRecepientDetails = () => {
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto">
      <h2 className="text-3xl font-bold text-center mb-10 text-[#111827]">All Recipients</h2>
  
      {user?.approved === true && !recepients?.length ? (
        <p className="text-center text-xl text-gray-500 mt-10">No Recipients Have Registered Yet!</p>
      ) : user?.approved === true && recepients?.length ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {recepients.map((recepient, i) => (
            ...
            >
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-[#1f2937] mb-1">Recipient #{i + 1}</h3>
              ...
              <div className="space-y-3 text-sm">
                {[
                  { label: "Frequency of Need", value: recepient.frequencyOfNeed },
                  { label: "Type Of Food", value: recepient.typeOfFood },
                  { label: "Quantity", value: recepient.quantity },
                  { label: "Receiving Option", value: recepient.receivingOption },
                  { label: "Preferred Time", value: recepient.prefferedPickUpTime },
                ].map((item, idx) => (
                  <div
                    key={idx}
                ...
                <div className="border border-[#d1d5db] bg-[#f0f4f8] rounded-md p-3">
                  <span className="text-gray-600 block mb-1">Request Description:</span>
